...
function NewPost() {
  ...
  const handlePostSubmit = async (e) => {
    ...
    try {
      if (id) {
        try {
          const res = await axios.put(`${import.meta.env.VITE_API_URL}/api/posts/${id}`, formData);
          if (res.status === 200) {  // Check if the response status is successful (200)
    ...
    <div className="p-5 h-screen">
      ...
      <div className="flex flex-col lg:flex-row gap-6 p-5">
        ...
        <ContentEditor
          ...
          handleEditorChange={(content) => setPostDetails((prev) => ({ ...prev, content }))}
...
const Sidebar = memo(({ postDetails, setPostDetails, categories, authors, featuredImage, setFeaturedImage, tags, setTags, tagInput, setTagInput }) => (
  <div className="lg:w-1/4 p-6 bg-gray-100 rounded-lg shadow-md">
    <ImageUploader featuredImage={featuredImage} setFeaturedImage={setFeaturedImage} />
    <PublishStatus postDetails={postDetails} setPostDetails={setPostDetails} />
    <CategorySelector categories={categories} postDetails={postDetails} setPostDetails={setPostDetails} />
    <TagsInput tags={tags} setTags={setTags} tagInput={tagInput} setTagInput={setTagInput} />
    <AuthorSelector authors={authors} postDetails={postDetails} handleInputChange={(e) => setPostDetails((prev) => ({ ...prev, author: e.target.value }))} />
  </div>
));

const ContentEditor = memo(({ postDetails, handleInputChange, handleEditorChange, handlePostSubmit, id }) => (
  <div className="lg:w-3/4 p-6 bg-gray-100 rounded-lg shadow-md">
    ...
    <div className="mb-4">
      <label className="block text-gray-700" htmlFor="title">SEO Title</label>
      <input
        type="text"
        name="seoTitle"
    ...
    <div className="mb-4">
      <label className="block text-gray-700" htmlFor="title">SEO Description</label>
      <input
        type="text"
        name="seoDescription"
    ...
    <div className="mb-4">
      ...
      <Editor value={postDetails.content} onChange={handleEditorChange} placeholder="Start typing here..." />
    </div>
    <div className="flex justify-end gap-4">
      <button onClick={handlePostSubmit} className="bg-blue-600 text-white px-6 py-2 rounded-md hover:bg-blue-700">
...
const PublishStatus = memo(({ postDetails, setPostDetails }) => (
  <div className="mt-4">
    ...
    <button
      ...
      className={`mt-2 w-full p-2 rounded-md ${postDetails.blogType === "draft" ? "bg-blue-600 text-white" : "bg-gray-300"}`}>
      Draft
    </button>
    <button
      onClick={() => setPostDetails((prev) => ({ ...prev, blogType: "published" }))}
      className={`mt-2 w-full p-2 rounded-md ${postDetails.blogType === "published" ? "bg-green-600 text-white" : "bg-gray-300"}`}>
      Publish
    </button>
  </div>
));
    ...
    <div className="flex flex-wrap gap-2 mt-2">
      {tags.map((tag, index) => (
        <span key={index} className="px-3 py-1 bg-gray-200 rounded-full text-sm">
          {tag} <button onClick={() => setTags(tags.filter((_, i) => i !== index))}>&times;</button>
        </span>
      ))}
    </div>
  </div>
...
const AuthorSelector = memo(({ authors, postDetails, handleInputChange }) => (
  <div className="mt-4">
    ...
    <select name="author" value={postDetails.author} onChange={handleInputChange} className="w-full p-2 border border-gray-300 rounded">
